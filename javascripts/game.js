...
costs={tiers:[],tupgs:[0,1,1,1,0,0,0,0,00,0,300,500,750,3000],snupgs:[1,15,300,1,1,1,2,2,3,4,5,6,8,9,10,12],intReduceCost:1,bisfeatures:[3000,5000,7500,10000,1e5,1e6],bbCost:1000,neutronBoosts:[0,0,0,0,0],neutronTiers:[]}
gainRate=[0,0]
...
function formatTime(s) {
	if (s < 0.01) {
		...
		return Math.floor(s*10)/10+' seconds'
	} else if (s < 36) {
		return Math.floor(s/6)+' minutes and '+Math.floor(s%6)+' seconds'
	} else if (s < 864) {
		return Math.floor(s/36)+' hours, '+Math.floor(s/6%6)+' minutes, and '+Math.floor(s%6)+' seconds'
	} else if (s < 26297.46) {
